...
cluster_name=$(curl -s -u $LOGIN:$PASSWORD --insecure "http://$AMBARI_HOST:$PORT/api/v1/clusters"  | python -mjson.tool | perl -ne '/"cluster_name":.*?"(.*?)"/ && print "$1\n"')
...
curl -s -u $LOGIN:$PASSWORD --insecure "http://$AMBARI_HOST:$PORT/api/v1/clusters/$cluster_name/hosts" | grep host_name | awk -F ':' '{print $2}' |  awk -F '"' '{print $2}' >  $intr/hosts.txt


while IFS= read -r host
do
os=$(curl -s -u $LOGIN:$PASSWORD --insecure "http://$AMBARI_HOST:$PORT/api/v1/clusters/$cluster_name/hosts/$host?fields=Hosts/os_type" | grep os_type | grep -v href | awk -F ':' '{print $2}' |  awk -F '"' '{print $2}')
...
running_components=$(curl -s -u $LOGIN:$PASSWORD --insecure "http://$AMBARI_HOST:$PORT/api/v1/clusters/$cluster_name/services?fields=ServiceInfo/service_name&ServiceInfo/maintenance_state=OFF" | python -mjson.tool | perl -ne '/"service_name":.*?"(.*?)"/ && print "$1\n"')
...
for s in $running_components; do
    ...
    curl -s -u $LOGIN:$PASSWORD --insecure -H "X-Requested-By:X-Requested-By" -X POST --data "$post_body"  "http://$AMBARI_HOST:$PORT/api/v1/clusters/$cluster_name/requests"
